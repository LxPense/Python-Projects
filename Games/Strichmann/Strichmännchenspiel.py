...
class Spiel:
    def __init__(self):
        ...
        w = self.bg.width()
        h = self.bg.height()
        zeichne_Hintergrund = 0
        for x in range(0, 5):
            for y in range(0, 5):
                if zeichne_Hintergrund == 1:
                    self.canvas.create_image(x * w, y * h, image=self.bg, anchor='nw')
                    zeichne_Hintergrund = 0
                else:
                    zeichne_Hintergrund = 1
        self.sprites = []
        self.rennen = True
...
def innerhalb_y(co1, co2):
    if (co1.y1 > co2.y1 and co1.y1 < co2.y2) \
        or (co1.y2 > co1.y1 and co1.y2 < co2.y2) \
...
class EbenenSprite(Sprite):
    def __init__(self, spiel, photo_image, x, y, width, height):
        ...
        self.image = spiel.canvas.create_image(x, y, image=self.photo_image, anchor='nw')
...
class TürSprite(Sprite):
    def __init__(self, spiel, photo_image, x, y, width, height):
        ...
        self.koordinaten = Koordinaten(x, y, x + (width / 2), y + height)


                    
s = Spiel()
Ebene1 = EbenenSprite(s, PhotoImage(file="Ebene2.gif"), 0, 480, 100, 10)
Ebene2 = EbenenSprite(s, PhotoImage(file="Ebene2.gif"), 150, 440, 100, 10)
Ebene3 = EbenenSprite(s, PhotoImage(file="Ebene2.gif"), 300, 400, 100, 10)
Ebene4 = EbenenSprite(s, PhotoImage(file="Ebene2.gif"), 300, 160, 100, 10)
Ebene5 = EbenenSprite(s, PhotoImage(file="Ebene3.gif"), 175, 350, 66, 10)
Ebene6 = EbenenSprite(s, PhotoImage(file="Ebene3.gif"), 50, 300, 66, 10)
Ebene7 = EbenenSprite(s, PhotoImage(file="Ebene3.gif"), 170, 120, 66, 10)
Ebene8 = EbenenSprite(s, PhotoImage(file="Ebene3.gif"), 45, 60, 66, 10)
Ebene9 = EbenenSprite(s, PhotoImage(file="Ebene1.gif"), 170, 250, 32, 10)
Ebene10= EbenenSprite(s,PhotoImage(file="Ebene1.gif"),  230, 200, 32, 10)
